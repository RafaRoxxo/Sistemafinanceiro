...
import { FORCE_RELOAD_VERSION } from "./force_reload.tsx";

// Updated: 2026-05-14 - All kv.delete replaced with kv.del - v${FORCE_RELOAD_VERSION}
const app = new Hono();
...
app.get("/make-server-808cc1b6/health", (c) => {
  return c.json({
    ...
    timestamp: Date.now(),
    message: "Server is running - kv.del fix applied"
  });
});
...
app.post("/make-server-808cc1b6/compra", authMiddleware, async (c) => {
  try {
    ...
    // Se vincular_pessoa foi marcado, criar dívida automaticamente
    if (data.vincular_pessoa) {
      // Suportar múltiplas pessoas (novo formato)
      const pessoasIds = data.pessoas_ids || (data.pessoa_id ? [data.pessoa_id] : []);

      if (pessoasIds.length > 0) {
        console.log(`🔵 Criando dívida para ${pessoasIds.length} pessoa(s)`);

        const dividaPaiId = crypto.randomUUID();
        const dividirIgualmente = data.dividir_igualmente ?? true;

        // Criar uma dívida para cada parcela restante
        for (let i = parcelaAtual; i <= totalParcelas; i++) {
          const mesOffset = i - parcelaAtual;
          const [ano, mes] = mesInicio.split("-").map(Number);
          const dataMes = new Date(ano, mes - 1 + mesOffset, 1);
          const anoCobranca = dataMes.getFullYear();
          const mesCobranca = String(dataMes.getMonth() + 1).padStart(2, "0");
          const dataDivida = `${anoCobranca}-${mesCobranca}-01`;

          // Calcular valores para cada pessoa
          let pessoas = [];
          if (dividirIgualmente) {
            const valorPorPessoa = data.valor_parcela / pessoasIds.length;
            pessoas = pessoasIds.map((pessoa_id: string) => ({
              pessoa_id,
            ...
            // Divisão manual
            pessoas = pessoasIds.map((pessoa_id: string) => {
              const valorIndividual = data.valores_individuais?.[pessoa_id]
                ? parseFloat(data.valores_individuais[pessoa_id])
                : 0;
              return {
                pessoa_id,
          ...
          const divida = {
            id: dividaId,
            usuario_id: userId,
            valor_total: data.valor_parcela,
            valor_parcela: data.valor_parcela,
            descricao: totalParcelas > 1
              ? `${data.descricao} (${i}/${totalParcelas})`
              : data.descricao,
            data: dataDivida,
            observacoes: "",
            pessoas,
          ...
          console.log(`  ✅ Dívida criada para parcela ${i}/${totalParcelas} - ID: ${dividaId}`);
        }

        console.log(`✅ Dívidas criadas para ${pessoasIds.length} pessoa(s) vinculada(s)`);
      }
    }

    // TODO: Implementar suporte para compra recorrente
    // Se data.recorrente for true, criar compras futuras com base na frequência
    if (data.recorrente && data.frequencia) {
      console.log(`🔵 Compra recorrente - Frequência: ${data.frequencia}`);
      console.log(`⚠️ Funcionalidade de compra recorrente será implementada em uma atualização futura`);
      // A lógica de recorrência pode ser implementada posteriormente
      // para criar automaticamente novas compras em meses futuros
    }
...
app.delete("/make-server-808cc1b6/parcela-v2/:id", authMiddleware, async (c) => {
  try {
    ...
    console.log(`  UserId: ${userId}, ParcelaId: ${id}, DeletarTodas: ${deletarTodas}`);
    ...
    if (deletarTodas && parcela.compra_id) {
      ...
      // Buscar e adicionar dívidas vinculadas
      ...
      // Buscar dívida vinculada a esta parcela específica
      const todasDividas = await kv.getByPrefix(`divida:${userId}:`);
      const dividaVinculada = todasDividas.find((d) =>
        d.compra_id === parcela.compra_id && d.numero_parcela === parcela.numero_parcela
      );

      if (dividaVinculada) {
        keysToDelete.push(`divida:${userId}:${dividaVinculada.id}`);
        console.log(`  Dívida vinculada encontrada: ${dividaVinculada.id}`);
      }
